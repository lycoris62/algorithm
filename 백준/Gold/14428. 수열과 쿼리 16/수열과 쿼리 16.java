...
public class Main {
	...
	static int min(int start, int end, int node, int left, long right) {
		...
		int mid = (start + end) / 2;
		int leftValue = min(start, mid, node * 2, left, right);
		int rightValue = min(mid + 1, end, node * 2 + 1, left, right);

		return getIndex(leftValue, rightValue);
	}
}
